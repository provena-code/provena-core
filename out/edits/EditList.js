...
import { Span, EditNode } from '../shared/edit-data';
...
export class EditList {
    constructor() {
        ...
        this.trace = (..._args) => { };
        this.logError = (..._args) => { console.error(..._args); };
    }
    getEdits() {
        return this.edits;
    }
    ...
    searchCurrentEdits(query) {
        ...
        return allIndices.map(startIndex => {
            ...
            while (editIndex < this.edits.length) {
                ...
                const rangeSubset = new Span(Math.max(edit.range.start, startIndex), Math.min(edit.range.end, endIndex));
    ...
    //     return undefined;
    // }
    findEditsWithinRange(span, ignoreAbutting = true) {
        const result = [];
    ...
    getAuthors(span, ignoreAbutting) {
    ...
    addEdit(changeEvent, metadata, isUndoOrRedo = false, pasteMatch = null) {
        ...
        if (text.length !== 0) {
            ...
            let matchPath = this.findUndoOrRedoMatch(isUndoOrRedo, index, subsequentEdit, text);
            if (matchPath) {
    ...
    insertQueryMatch(rangeStart, matchPath, updateTime, insertionIndex) {
    ...
    findUndoOrRedoMatch(isUndoOrRedo, index, subsequentEdit, text) {
        if (!isUndoOrRedo) {
            return null;
        }
        // TODO: Handle index = 0
    ...
    defragment() {
        for (let i = 0; i < this.edits.length - 1; i++) {
            ...
            if (current.range.end === next.range.start &&
                ...
                const mergedEdit = new EditNode(new Span(current.range.start, next.range.end), current.text + next.text, {
    ...
    shiftEdits(start, replacedRange, text) {
